  ...
  useEffect(() => {
    req('GET', '/user/product')
      .then(({products}) => {
        ...
        setTemplateArray(products?.filter(prod => prod.backgrounds.length)?.map((p, id) => { return {...p, id, image: p.defaultIllustration ? {url: p.defaultIllustration} : p.backgrounds[p.defaultBackground]} }))
      })
  }, [])
